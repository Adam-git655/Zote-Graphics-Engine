...
unsigned int Primitives::createCubeUnlitVAO()
{
	...
	glBindVertexArray(VAO);
	glBindBuffer(GL_ARRAY_BUFFER, VBO);
	glBufferData(GL_ARRAY_BUFFER, sizeof(unlitCubeVertices), unlitCubeVertices, GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unlitCubeIndices), unlitCubeIndices, GL_STATIC_DRAW);
